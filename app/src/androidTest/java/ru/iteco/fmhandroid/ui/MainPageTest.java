...
public class MainPageTest {
    ...
    @Test
    @Flaky
    @DisplayName(value = "Тест-кейс 23. Отображение трех последних новостных постов")
    @io.qameta.allure.kotlin.Description(value = "Тест проверяет отображение max кол-во постов в боксе")
    public void maxNewsInMainPageTest() {
        Allure.step("Шаг 2: Посмотреть отображаемые посты");
        ViewInteraction comboBox = onView(withId(R.id.news_list_recycler_view));
        comboBox.check(matches(isDisplayed()));
        comboBox.check(matches(CustomViewMatcher.recyclerViewSizeMatcher(3)));
